...
struct hole *findBiggestHole(phys_clicks clicks, struct hole **prev_biggest_hole_ptr)
{
 ...
 */
  ...
  struct hole * prev_biggest_hole;
  struct hole * worst_hole;
  ...
  do {
	...
	/* worst-fit allocation */
		worst_hole = findBiggestHole(clicks, &prev_biggest_hole);
...

{
	...
	{
		buffer[count * 2] = hp->h_len;
		buffer[count * 2 + 1] = hp->h_base;
		++count;
	}
